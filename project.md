...
### Dashboard / Widgets
- [ ] Configure Google API keys (Calendar widget) — `VITE_GOOGLE_CLIENT_ID` set, but full OAuth flow not yet exercised end-to-end
- [ ] Deploy `google-oauth-exchange` Edge Function
- [ ] Test full widget persistence and OAuth flow in production
...
### 2026-05-11 (2) — Landing page deepening: OG image, semantic HTML, copy, CTA, perf
- `public/og-image.png` (new, 1200×630) + `public/og-image.svg` source. Generated by `scripts/generate-og-image.mjs` (run with `node scripts/generate-og-image.mjs`; uses `sharp` via `npm install --no-save sharp`). Social cards now have a real visual.
- `LandingPage.tsx` (FeatureCard): the four feature words are now real `<h2>` and the subheads `<h3>` — gives crawlers a structural outline (was just styled `<span>`s).
- `LandingPage.tsx` (hero): added a visible "Start a conversation" primary CTA below the subtitle. Opens Agent Stone via the new controlled-open prop. `LandingAgent.tsx` accepts optional `open`/`onOpenChange` props for controlled mode; falls back to its internal state when unprovided.
- `LandingPage.tsx` (new "How we work" section): ~250 words of crawlable copy below the fold — what stonecode.ai is, how engagements work, what we build (RAG, agentic workflows on Claude/OpenAI, Supabase/Postgres platforms, multi-tenant SaaS). Two CTAs at the end: "Talk to Andrew" (opens Agent Stone) and "Email us".
- **Perf:** moved `WidgetProvider` from `main.tsx` into `PortalLayout.tsx` so widget code no longer ships in the landing entry chunk. Added Vite `manualChunks` to split `@supabase/supabase-js` into its own cacheable chunk. Result: entry chunk went 464 kB → 290 kB (38% smaller, faster TTI); Supabase is now a parallel-loadable, long-cacheable chunk shared across landing + portal. Total first-paint bytes roughly unchanged (~153 kB gz), but smaller entry parses faster and per-vendor caching wins on every subsequent deploy.
