...
const NOTE_COLORS = [
  { id: 'slate', bg: 'bg-slate-100 dark:bg-slate-700', border: 'border-slate-200 dark:border-slate-600' },
  { id: 'violet', bg: 'bg-violet-50 dark:bg-violet-900/50', border: 'border-violet-200 dark:border-violet-700' },
  { id: 'blue', bg: 'bg-blue-50 dark:bg-blue-900/50', border: 'border-blue-200 dark:border-blue-700' },
  { id: 'green', bg: 'bg-green-50 dark:bg-green-900/50', border: 'border-green-200 dark:border-green-700' },
  { id: 'amber', bg: 'bg-amber-50 dark:bg-amber-900/50', border: 'border-amber-200 dark:border-amber-700' },
  { id: 'rose', bg: 'bg-rose-50 dark:bg-rose-900/50', border: 'border-rose-200 dark:border-rose-700' },
]
                  ...
                  {editingId === note.id ? (
                    <div>
                      <textarea
                        ...
                        className="w-full bg-transparent text-slate-900 dark:text-white text-sm resize-none focus:outline-none min-h-[60px]"
                        ...
                        <div className="flex gap-1">
                          {NOTE_COLORS.map((c) => (
                            <button
                              ...
                              className={`w-4 h-4 rounded-full ${c.bg} border ${c.border} ${note.color === c.id ? 'ring-2 ring-orange-500 dark:ring-white ring-offset-1 ring-offset-white dark:ring-offset-slate-800' : ''}`}
                            />
                          ))}
                        </div>
                    ...
                    <div>
                      <p
                        className="text-sm text-slate-900 dark:text-white whitespace-pre-wrap cursor-pointer min-h-[20px]"
                        onClick={() => {
                      ...
                      {confirmDeleteId === note.id ? (
                        <div className="absolute top-2 right-2 flex items-center gap-1 bg-white dark:bg-slate-800 rounded-lg px-2 py-1 border border-slate-200 dark:border-slate-600 shadow-sm dark:shadow-none">
                          <span className="text-xs text-slate-600 dark:text-slate-300">Delete?</span>
                          <button onClick={() => deleteNote(note.id)} className="text-xs text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 px-1">Yes</button>
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-opacity"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
